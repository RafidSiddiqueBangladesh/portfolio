...
export default function App({ apiKey }) {
  ...
  return (
    <div className="font-sans text-gray-900 min-h-screen flex items-center justify-center">
      ...
      <div className="flex w-full max-w-xl mx-auto items-center justify-center p-4">
        <div className="flex flex-1 gap-2 rounded-full p-2 border border-blue-500 shadow-lg bg-white">
          <input
            className="flex-1 p-2 rounded-full focus:outline-none text-gray-800"
          ...
          <button
            onClick={handleSend}
            className={`bg-blue-600 text-white px-5 py-2 rounded-full hover:bg-blue-700 transition font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
            disabled={isLoading}
          >
      ...
      {/* Floating Button to toggle the chat widget. It is positioned at the bottom by default and in the middle on larger screens (xl). */}
      <button
        onClick={() => setOpen(!open)}
        className="fixed top-1/2 -translate-y-1/2 right-6 z-50 xl:bottom-6 xl:top-auto xl:transform-none bg-blue-600 text-white px-4 py-2 rounded-full shadow-lg hover:bg-blue-700 transition font-semibold focus:outline-none focus:ring-4 focus:ring-blue-300"
      >
      ...
      {open && (
        <div className="fixed top-1/2 -translate-y-1/2 right-5 z-50 xl:bottom-20 xl:top-auto xl:transform-none w-[300px] h-[400px] sm:w-[300px] bg-white shadow-2xl rounded-2xl border border-gray-300 p-4 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 mb-3 pr-2 text-sm">
            {messages.map((msg, i) => (
              <div
                key={i}
                className={`max-w-[85%] break-words px-3 py-2 rounded-2xl shadow-sm ${
                  msg.sender === "user"
            ...
            {/* Loading indicator */}
            {isLoading && (
              <div className="bg-gray-200 text-gray-900 px-3 py-2 rounded-2xl shadow-sm w-fit mr-auto">
          ...
          <div className="flex gap-2">
            <input
              className="flex-1 border border-gray-300 p-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800 transition duration-200 text-sm"
            ...
            <button
              onClick={handleSend}
              className={`bg-blue-600 text-white px-4 rounded-xl hover:bg-blue-700 transition font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
              disabled={isLoading}
            >
